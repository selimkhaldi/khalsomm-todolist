...
const configSchema = Joi.object().keys({
    ...
    rules_verify_signature: Joi.boolean().default(DEFAULT.rules_verify_signature),
    log_level: Joi.string().only(LOG_LEVELS).default(DEFAULT.log_level),
    log_location: Joi.string().default(DEFAULT.log_location),
    run_in_test: Joi.boolean().default(DEFAULT.run_in_test),
    block_all_rules: Joi.boolean().default(DEFAULT.block_all_rules),
    report_perf_newrelic: Joi.boolean().default(DEFAULT.report_perf_newrelic)
});
...
const parseConfig = function (rawConfig) {
    ...
    if (config.local_rules) {
        try {
            ...
            Logger.ERROR(`Could not load local rules ${err}`);
        }
    }
