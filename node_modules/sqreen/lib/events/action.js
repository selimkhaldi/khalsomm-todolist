 ...
 * @returns {*}
 */
module.exports.trigger = function (eventQueue, retryTimeout, force) {

    retryTimeout = retryTimeout || 1000 * 100;
    ...
    for (let i = 0; i < eventQueue.length; ++i) {
        ...
        promiseQueue.push(reportEvent(event)
            .catch(() => {
                ...
                // if an err happened: we do not report it (it might lead to an infinite report loop !
                // we try to report again after a while and
                return Utils.timeout(retryTimeout * Math.random())
                    .then(() => reportEvent(event));
            })
            .catch(() => {
