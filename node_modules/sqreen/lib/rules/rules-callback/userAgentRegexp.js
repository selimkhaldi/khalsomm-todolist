...
module.exports.getCbs = function (rule_) {
    ...
    const matcherList = rule_.data.values.map((val) => ({ matcher: RegexpRule.getRegexpMatcher(val), pattern: val }));
        ...
        pre: function (args, value, rule) {
            ...
            if (args[0] === 'request' && args[1] instanceof Http.IncomingMessage && args[2] instanceof Http.ServerResponse) {
                ...
                for (let i = 0; i < matcherList.length; ++i) {
                    ...
                    if (matcherList[i].matcher(userAgent)) {

                        const result = { record: { found: matcherList[i].pattern }, originalSession: { req: args[1], res: args[2] } };
                        if (rule.block) {
